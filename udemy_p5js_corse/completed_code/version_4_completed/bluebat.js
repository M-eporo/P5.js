class Bluebat{
    constructor(img){   // const bluebat = new Bluebat(bluebatImage)
        this.img = img;
        this.originalW = this.img.width/11;
        this.originalH = this.img.height
        this.x = width + random()*width*3;
        // this.y = map(random(), 0, 1, height/10, height*9/10);
        this.noiseValue = random();   // noise()の初期値
        this.y = height * noise(this.noiseValue);
    ...
    update(){
        this.x += this.speedX;
        this.noiseValue += 0.01;   // 増加量が大きくなればnoiseの変化量も大きくなる
        this.y = height * noise(this.noiseValue);
        // this.y += this.speedY;
        this.cX = this.x - this.centerOffset;
        this.cY = this.y + this.centerOffset;
